    ...
    <div
      ...
      className={`group relative flex items-center justify-center overflow-hidden text-center transition-all duration-350 ease-[cubic-bezier(0.4,0,0.2,1)] rounded-[clamp(14px,2vw,20px)] border-2 ${
        preview 
          ...
          : 'p-[clamp(36px,7vw,56px)_clamp(20px,4vw,32px)] border-dashed min-h-[220px] bg-white/5 ' + 
            (dragOver 
      ...
      {preview ? (
        <>
          ...
          <button 
            ...
            className="absolute top-3 right-3 px-3.5 py-1.5 rounded-full bg-black/55 backdrop-blur-sm border border-white/10 text-white/80 text-[clamp(11px,2vw,12px)] font-medium cursor-pointer transition-all duration-200 hover:bg-red-500/50 hover:border-red-500/40 hover:text-white z-30"
          >
            ✕ Remove
          </button>
        </>
      ) : (
        <div className="w-full animate-fade-in-up pointer-events-none relative z-10">
